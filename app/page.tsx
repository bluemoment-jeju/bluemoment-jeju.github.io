...
export default function Home() {
  return <main>
    ...
    <section className="store-section"><div className="store-kicker">STAY UPSTAIRS · SHOP DOWNSTAIRS</div><div className="store-content"><p className="section-index">05 · 24H STORE</p><h2>한 층 아래,<br/>24시간 달시.</h2><p>블루모먼트와 같은 건물 1층에는 무인편의점 ‘달시’가 있습니다. 과자, 음료, 아이스크림과 주류, 마른안주부터 물놀이용품과 생활용품까지 늦은 밤에도 부담 없는 가격으로 이용할 수 있습니다. 라면 조리 공간에서는 월령리 바다를 바라보며 따뜻한 한 끼도 즐겨보세요.</p><div className="store-tags"><span>SNACK</span><span>DRINK</span><span>BEER</span><span>ICE CREAM</span><span>RAMEN</span><span>24 HOURS</span></div></div><div className="store-gallery">{[1,2,3,4,5,6].map((number)=><figure key={number}><img src={`/images/dalsi-0${number}.jpg`} alt={`블루모먼트 1층 무인편의점 달시 ${number}`}/></figure>)}</div></section>
